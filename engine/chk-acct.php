<?php 
    ...
    function chk30($conn){
        ...
        $getOffice = $conn->prepare("SELECT * FROM offices a JOIN users b USING(userID) WHERE a.officeExpiry BETWEEN ? AND ? AND a.officeStatus = ?");
        ...
        if($officesct > 0){
            foreach($offices AS $office){
                ...
                $msg1 = "<html><body style='font-size:14px'> <p> <img src='https://eoffice.ng/assets/images/eoffice-expiration.jpg' style='width:100%' /> </p> <h2 align='center'> 30 days to go! </h2> <p> Hello ".$office['fname'].", </p><p> This is to notify you that your eOffice with address https://".$office['officeDomain'].".eoffice.ng will expire on <b>".date('d M, Y', $office['officeExpiry'])."</b>. </p> <p>This means your eoffice will be unpublished on the above date and will no longer be accessible to the general public.</p> <p>To renew, please login to your dashboard with this link <a href='https://www.eoffice.ng/dashboard'>www.eoffice.ng/dashboard</a>. </p> <p>We look forward to serving you always!</p> <p>Warm Regards! <br> eOffice NG Sales Team. </p> </body></html>";
